...
// Where adds a new predicate for the ReviewUpdate builder.
...
func (ru *ReviewUpdate) check() error {
	...
	if _, ok := ru.mutation.AuthorID(); ru.mutation.AuthorCleared() && !ok {
		return errors.New("gen: clearing a required unique edge \"author\"")
	}
	if _, ok := ru.mutation.ProductID(); ru.mutation.ProductCleared() && !ok {
		return errors.New("gen: clearing a required unique edge \"product\"")
	}
	return nil
}
