...
func (rc *ReviewCreate) check() error {
	if _, ok := rc.mutation.Body(); !ok {
		return &ValidationError{Name: "body", err: errors.New(`gen: missing required field "Review.body"`)}
	}
	...
	if _, ok := rc.mutation.AuthorID(); !ok {
		return &ValidationError{Name: "author", err: errors.New(`gen: missing required edge "Review.author"`)}
	}
	if _, ok := rc.mutation.ProductID(); !ok {
		return &ValidationError{Name: "product", err: errors.New(`gen: missing required edge "Review.product"`)}
	}
	return nil
}
