...
func (ru *ReviewUpdate) check() error {
	...
	if _, ok := ru.mutation.AuthorID(); ru.mutation.AuthorCleared() && !ok {
		return errors.New(`entgen: clearing a required unique edge "Review.author"`)
	}
	if _, ok := ru.mutation.ProductID(); ru.mutation.ProductCleared() && !ok {
		return errors.New(`entgen: clearing a required unique edge "Review.product"`)
	}
	return nil
}
...
func (ruo *ReviewUpdateOne) sqlSave(ctx context.Context) (_node *Review, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`entgen: missing "Review.id" for update`)}
	}
